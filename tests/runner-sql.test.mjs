...
async function fixture(t) {
  ...
  await client.executeMultiple(
    readFileSync(
      new URL('../migrations/007_runner.sql', import.meta.url),
      'utf8',
    ),
  );
  await client.executeMultiple(
    readFileSync(
      new URL('../migrations/008_computer_use.sql', import.meta.url),
      'utf8',
    ),
  );
  await client.executeMultiple(
...
async function queued(runtime, device, overrides = {}) {
  return store.queueJob(runtime, device.ownerId, {
    businessId: 'biz',
    endeavorId: 'endeavor',
    deviceId: device.deviceId,
    revision: 1,
    brief: 'Draft a plan. Do not publish.',
    ...overrides,
  });
}
...
test('pairing is one-time, expiring, hashed, owner-bound, and revocable', async (t) => {
  ...
  await assert.rejects(
    store.redeemPairCode(runtime, code.code, 'Second laptop'),
    /invalid|expired|used/i,
  );
  assert.deepEqual(await store.authenticateDevice(runtime, device.token), {
    deviceId: device.deviceId,
    ownerId: 'alice',
  });
  assert.equal(await store.authenticateDevice(runtime, 'invalid-token'), null);
  const persisted = await client.execute(
    'SELECT token_hash FROM runner_devices',
  );
  assert.notEqual(persisted.rows[0].token_hash, device.token);
  assert.equal((await store.listRunner(runtime, 'bob')).devices.length, 0);
  assert.equal(
    await store.revokeDevice(runtime, 'bob', device.deviceId),
    false,
  );
  assert.equal(
    await store.revokeDevice(runtime, 'alice', device.deviceId),
    true,
  );
  ...
  await assert.rejects(
    store.redeemPairCode(runtime, expired.code, 'Late laptop'),
    /invalid|expired/i,
  );
});
...
test('failed device creation rolls back pairing consumption', async (t) => {
  ...
  const code = await store.createPairCode(runtime, 'alice');
  await client.executeMultiple(
    "CREATE TRIGGER reject_test_device BEFORE INSERT ON runner_devices BEGIN SELECT RAISE(ABORT, 'test failure'); END;",
  );
  await assert.rejects(
    store.redeemPairCode(runtime, code.code, 'Laptop'),
    /test failure/,
  );
  await client.executeMultiple('DROP TRIGGER reject_test_device;');
  assert.ok(
    (await store.redeemPairCode(runtime, code.code, 'Laptop')).deviceId,
  );
});
...
test('expired jobs never replay and cancellation rejects late results', async (t) => {
  ...
  await client.execute({
    sql: 'UPDATE runner_jobs SET lease_expires_at=? WHERE id=?',
    args: ['2000-01-01T00:00:00.000Z', claim.job.id],
  });
  assert.equal(await store.claimJob(runtime, a.deviceId, a.ownerId), null);
  assert.equal(
    (await store.listRunner(runtime, a.ownerId)).jobs[0].status,
    'failed',
  );
  assert.equal(
    await store.finishJob(
...
test('approval decisions are owner-only, immutable, scoped to a job, and expire with the lease', async (t) => {
  ...
  assert.equal(
    ...
    null,
    'a runner cannot grant itself permission',
  );
  assert.equal(
  ...
  await store.recordEvent(
    ...
    first.job.id,
    first.leaseToken,
    { ...event, details: { command: 'different command' }, decision: 'denied' },
  );
  decisions = (
    await store.heartbeat(
      runtime,
  ...
  await client.execute({
    sql: 'UPDATE runner_jobs SET lease_expires_at=? WHERE id=?',
    args: ['2000-01-01T00:00:00.000Z', second.job.id],
  });
  assert.equal(
    await store.decideApproval(
      runtime,
...
test('revocation stops claims, events, heartbeats, and result settlement', async (t) => {
  ...
  const beat = await store.heartbeat(
    runtime,
    a.ownerId,
    a.deviceId,
    claim.job.id,
...
test('computer jobs require an explicitly capable paired device', async (t) => {
  const { runtime } = await fixture(t);
  const code = await store.createPairCode(runtime, 'alice');
  const ordinary = await store.redeemPairCode(runtime, code.code, 'ordinary');
  await assert.rejects(
    queued(runtime, ordinary, {
      executionMode: 'computer',
      goal: 'Open the project.',
    }),
    /not paired/i,
  );
  const capableCode = await store.createPairCode(runtime, 'alice');
  const capable = await store.redeemPairCode(
    runtime,
    capableCode.code,
    'capable',
    { computerUse: true },
