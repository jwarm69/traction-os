...
export async function POST(req: Request) {
  try {
    ...
    return Response.json({ ok: true }, { headers: { 'Set-Cookie': await register(runtime(), req, body.username, body.pin), 'Cache-Control': 'no-store' } });
  } catch (e) { return Response.json({ error: e instanceof Error ? e.message : 'Could not create account.' }, { status: 400 }); }
}
